...
st.write("""<h1 style='text-align: center; font-size: 13mm; color: rgba(0,255,0,0.8);'>Detect Deforestration</h1></br>
<p style='text-align: center; font-size: 6mm; color: rgba(0,255,0,0.8);'>Faculty of Science</br>University of Peradeniya</p></b>""", unsafe_allow_html=True)


if 'unet' not in st.session_state:
  st.session_state.unet = load_model(file_path + 'Unet_epoch_50')
if 'cnn' not in st.session_state:
  st.session_state.cnn = load_model(file_path + 'ResNet_epoch_50')
...
images = st.file_uploader("Select/drag and drop, satellite image or images : ", accept_multiple_files=True, type = ['png','jpg'])
  ...
  if num_images_now != st.session_state.num_images_prev:
    ...
    if submit:
      ...
      for image in images:
        ...
        time = time + patches.shape[0]*patches.shape[1]/3
